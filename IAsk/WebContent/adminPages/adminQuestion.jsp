				...
				<div class="group">
					<form action="QuestionSearchServlet" name="" method="post">
						<input class="searchk" type="text" class="form-control"
							placeholder="请输入关键字" name="id"> <input type="submit"
							value="查找">
					</form>
				</div>
					...
					<c:forEach var="Q" items="${QuestionAll}">
						<tr>
							...
							<td style="padding: 10px;"><a
								href="QuestionDeleteServlet?qId=${Q.quesId}"><button
										class="delete" onclick="del(this)">删除</button></a> <a
								href="QuestionTopServlet?qId=${Q.quesId}"><button
										class="topp" onclick="top(this)">置顶</button></a> <a
								href="QuestionUntopServlet?qId=${Q.quesId}"><button
										class="untop" onclick="unt(this)">取消</button></a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
