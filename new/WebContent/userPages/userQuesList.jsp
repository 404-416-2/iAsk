				...
				<div id="avatar" class="touxiang" class="nav nav-tabs">
					<div class="dropdown">
						...
						<div class="dropdown-content">
							<a href="<%=path%>/userQuesManagement">提问管理</a> <a
								href="<%=path%>/ViewUserAnswer">回答管理</a> <a
								href="<%=path%>/login.jsp">退出登录</a>
						</div>
					</div>
				...
				<tbody>
					...
					<c:forEach var="Q" items="${usrAllQues}">
						<tr>
							...
							<td>${Q.follow}</td>
							<td>${Q.starNum}</td>
							<td><c:if test="${Q.isTop > 0}">
									<p>是</p>
								</c:if> <c:if test="${Q.isTop < 1}">
									<p>否</p>
								</c:if></td>
							<td>${Q.category}</td>
							<td style="padding: 10px;"><a
								href="deleteQues?q_id=${Q.quesId}&u_id=${Q.usrId}"><button
										class="delete" onclick="del(this)">删除</button></a> <a
								href="userPages/updateQues.jsp?q_id=${Q.quesId}&u_id=${Q.usrId}&q_tx=${Q.quesText}&cate=${Q.category}"><button
										class="update">修改</button></a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
